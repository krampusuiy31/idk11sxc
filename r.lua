...
do
	Library = {
		Open = true;
		Accent = Color3.fromRGB(211, 211, 211);
		Pages = {};
		Sections = {};
		Flags = {};
		UnNamedFlags = 0;
	...
	do
		...
		function Library:Watermark(Properties)
			local Watermark = {
				Name = (Properties.Name or Properties.name or "krampus.space | placeholder");
				AnimateText = nil;
			}
			--
			local Outline = Instance.new("Frame")
			...
			task.spawn(function()
				while task.wait() do
					for i = 1, #"krampus.space" do
						Watermark.AnimateText = string.sub("krampus.space", 1, i) .. "";
						Title.Text = Watermark.AnimateText .. " " .. Watermark.Name;
						task.wait(0.4);
					end;

					for i = #"krampus.space" - 1, 1, -1 do
						Watermark.AnimateText = string.sub("krampus.space", 1, i) .. "";
						Title.Text = Watermark.AnimateText .. " " .. Watermark.Name;
						task.wait(0.4);
					end;
...
--// window locals
local default_color = Color3.fromRGB(211, 211, 211);
...
screen_gui.Name = "https://krampus.space";
...
screen_gui_2.Name = "https://krampus.space";
...
do
	...
	do
		...
		instances["local_text"] = utility.instance_new("TextLabel", { 
			Name = "https://krampus.space",
			Parent = screen_gui_2,
			Text = '<font color="rgb(211, 211, 211)">krampus</font><font color="rgb(255, 255, 255)">.space</font>',
		...
		do
			...
			utility.new_connection(run_service.Heartbeat, function()
				...
				if (enabled) then
					...
					local text = custom_text_enabled and '<font color="' .. text_color .. '">' .. features.get_text(custom_text) .. '</font>' or '<font color="' .. text_color .. '">krampus</font><font color="rgb(255, 255, 255)">.space</font>';
	...
	if (hookmetamethod) then
		local __namecall; __namecall = hookmetamethod(game, "__namecall", LPH_NO_VIRTUALIZE(function(self, ...)
			...
			if not checkcaller() and method == "Raycast" then
				if (flags["legit_silent_enabled"] and not flags["legit_silent_anti_aim_viewer"] and (locals.silent_aim.is_targetting and locals.silent_aim.target)) then
					args[2] = CFrame.lookAt(args[1], locals.silent_aim.predicted_position).LookVector * 5000;
				end;

					if (flags["rage_target_aim_enabled"] and (locals.target_aim.is_targetting and locals.target_aim.target)) then
						args[2] = CFrame.lookAt(args[1], locals.target_aim.predicted_position).LookVector * 5000;
					end;
...
do
	...
	do
		...
		do
			...
			option_list_toggle:Toggle({Name = "UI", Flag = "rage_target_aim_visuals_ui_enabled", Default = true, Callback = function(state)
				if state then return end;
	...
	do
		...
		do
			...
			c_sync:List({Name = "Type", Flag = "anti_aim_c_sync_type", Options = {"Static Local", "Static Target", "Local Random", "Target Random"}, Default = "Static Local"});
	...
	do
		...
		if not isfolder("krampus/configs") then 
		    makefolder("krampus/configs");
		end;

		local function update_config_list()
		    local list = {};
		
		    for idx, file in listfiles("krampus/configs") do
		        local file_name = file:gsub("krampus/configs\\", ""):gsub(".cfg", ""):gsub("krampus/configs/", "");
		...
		cfgs:Button({Name = "Create", Callback = function()
		    ...
		    if config_name == "" or isfile("krampus/configs/" .. config_name .. ".cfg") then
		        return;
		    end;
		    writefile("krampus/configs/" .. config_name .. ".cfg", Library:GetConfig());
		    update_config_list();
		end});
		...
		window:Toggle({Name = "Watermark", Flag = "ui_watermark", Default = true, Callback = function(state)
